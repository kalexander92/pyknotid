...
class PlanarDiagram(list):
    ...
    def jones_optimised(self, **kwargs):
        ...
        web.append([0, 1, diag_1])
        web.append([0, -1, diag_2])
        for crossing in crossings[1:]:
            crossing = PlanarDiagram(crossing)
            diag_1, diag_2 = kauffman_skein(crossing)
            new_web = []
            for term in web:
                diag_1_product = PlanarDiagram([x for x in diag_1] + [x for x in term[2]]).contract_points()
                diag_2_product = PlanarDiagram([x for x in diag_2] + [x for x in term[2]]).contract_points()
                contracted_1 = 0
                contracted_2 = 0
                diag_1_new = []
                diag_2_new = []
                ...
                new_web.append([term[0] + contracted_1, term[1] + 1, diag_1_new])
                new_web.append([term[0] + contracted_2, term[1] - 1, diag_2_new])
            web = new_web
        replacement = -A**2 - A**-2
        jones_precursor = 0
        for term in web:
            jones_precursor += replacement**term[0] * A**term[1]
        jones_poly = (-A**3)**self.writhe() * jones_precursor / replacement
        if root_of_unity is None:
            return sym.expand(sym.simplify(jones_poly)).subs(A, q**0.25)
        else:
